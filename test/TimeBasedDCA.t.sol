...
contract TimeBasedDCATest is BaseTest {
  ...
  function setUp() public override {
    ...
    dcaHook = new KSTimeBasedDCAHook(initialRouters);
  ...
  function test_exceedNumSwaps(uint256 seed) public {
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      if (i == timestamps.length - 1) {
        vm.expectRevert(
          abi.encodeWithSelector(KSTimeBasedDCAHook.ExceedNumSwaps.selector, numSwaps, i)
        );
      }
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      vm.expectRevert(
        abi.encodeWithSelector(
          KSTimeBasedDCAHook.InvalidExecutionTime.selector,
          timestamps[i],
  ...
  function test_invalidTokenIn(uint256 seed) public {
    ...
    KSTimeBasedDCAHook.DCAHookData memory hookData =
      abi.decode(intentData.coreData.hookIntentData, (KSTimeBasedDCAHook.DCAHookData));
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      vm.expectRevert(
        abi.encodeWithSelector(
          KSTimeBasedDCAHook.InvalidTokenIn.selector, hookData.srcToken, tokenIn
        )
      );
      router.execute(intentData, daSignature, guardian, gdSignature, actionData);
      vm.stopPrank();
  ...
  function test_invalidAmountIn(uint256 seed) public {
    ...
    KSTimeBasedDCAHook.DCAHookData memory hookData =
      abi.decode(intentData.coreData.hookIntentData, (KSTimeBasedDCAHook.DCAHookData));
    hookData.amountIn = 1e8; //invalid amountIn
    intentData.coreData.hookIntentData = abi.encode(hookData);
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      vm.startPrank(caller);
      vm.expectRevert(
        abi.encodeWithSelector(KSTimeBasedDCAHook.InvalidAmountIn.selector, 1e8, amountIn)
      );
      router.execute(intentData, daSignature, guardian, gdSignature, actionData);
      vm.stopPrank();
    }
  ...
  function test_invalidAmountOut_min(uint256 seed) public {
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      vm.expectRevert(
        abi.encodeWithSelector(
          KSTimeBasedDCAHook.InvalidAmountOut.selector, minAmountOut, maxAmountOut, actualAmountOut
        )
      );
      router.execute(intentData, daSignature, guardian, gdSignature, actionData);
      vm.stopPrank();
  ...
  function test_swapAlreadyExecuted(uint256 seed) public {
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      if (i == swapNo) {
        ...
        vm.expectRevert(abi.encodeWithSelector(KSTimeBasedDCAHook.SwapAlreadyExecuted.selector));
        router.execute(intentData, daSignature, guardian, gdSignature, actionData);
      }
      vm.stopPrank();
    }
  ...
  function _getIntentData() internal view returns (IntentData memory intentData) {
    ...
    hookData.amountOutLimits = (minAmountOut << 128) | maxAmountOut;
  ...
  function _setUpMainAddress(IntentData memory intentData, bool withSignedIntent) internal {
    ...
    IERC20(tokenIn).safeIncreaseAllowance(address(router), type(uint128).max);
    if (!withSignedIntent) {
