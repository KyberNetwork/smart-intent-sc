...
contract DCATest is BaseTest {
  ...
  function setUp() public override {
    ...
    timestamps.push(uint32(1_742_449_139));
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      vm.expectRevert(
        abi.encodeWithSelector(
          KSDCAIntentValidator.InvalidExecutionTime.selector, timestamps[i] + 120, timestamps[i]
        )
      );
  ...
  function test_timeBasedFailed_wrongAmountOut(uint256 mode) public {
    ...
    for (uint256 i; i < timestamps.length; i++) {
      ...
      vm.expectRevert(
        abi.encodeWithSelector(
          KSDCAIntentValidator.BelowOutputAmount.selector, 10e22, actualAmountOut
        )
      );
